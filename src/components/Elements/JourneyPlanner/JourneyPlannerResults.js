    ...
    {journeyData.journeys.slice(0, 5).map((journey, index) => {
      return <div key={index} className={styles.ResultsDiv}>
        <div className={styles.ResultsDivContent}>
          ...
          <a className={styles.routeDetailsButton} onClick={() => showRouteDetails(index)}>View Route Details<i className="fa-solid fa-angle-down"></i></a>
        </div>
        
        {journey.legs.map((legs, pos) => {
          return (activeIndex === index) && showRouteDetail ? <div key={pos} className={`${styles.back} ${legs.mode.name === "walking" || legs.mode.name === "bus" || legs.mode.name === "coach" ? styles[legs.mode.name] : styles[legs.routeOptions[0].name.replace(/\s/g,"-")]}`}>
